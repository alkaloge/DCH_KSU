...
'''
cats = ['eeee','eeem','eeet','eemm','eemt','eett','mmmm','mmem','mmmt','mmtt','mmet','tttt','ttet','ttmt','ttem','emem','emet','emmt','etet','etmt','mtmt']#ggr
...
for count, e in enumerate( inTree) :
	...
	#for dch1 in ['mm','ee'] :
	    ...
	    lepListM=[]
            ...
            signC=0
            netS=-99
	    if dch1=='mm' and len(goodMuonList) < 2 :
		if printOn:   print dch1, e.run, e.luminosityBlock,  e.event, 'failed as you have < 2 goodMuonList'
		continue

	    if dch1=='ee' and len(goodElectronList) < 2 :
		if printOn:   print dch1, e.run, e.luminosityBlock,  e.event, 'failed as you have < 2 goodElectronList'
		continue
